...
class RdmaHw : public Object {
   public:
    ...
    std::string m_monFilePrefix;

    // qp complete callback
    typedef Callback<void, Ptr<RdmaQueuePair>> QpCompleteCallback;
    QpCompleteCallback m_qpCompleteCallback;

	void SetMonFilePrefix(std::string mon_file_prefix);
     ...
     *  INT CC
     ***********************/
    uint32_t m_maxStage = 5;
    // 用于标识WAN流的利用率是否在目标范围内
    double m_epsilon = 0.01;
    bool IsDCFlow(Ptr<RdmaQueuePair> qp, CustomHeader &ch);
    void HandleAckINTCC(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch);
    void ComptUtilization(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch);
    void UpdateRateIntra(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch, bool fast_react);
    void UpdateRateCross(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch, bool fast_react);
