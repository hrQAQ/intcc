...
// printf("%.6lf\n", q->mlx.m_alpha);
	...
	void RdmaHw::HandleAckINTCC(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch)
	{
		uint32_t seq = ch.ack.seq;
	...
	void RdmaHw::ComptUtilization(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch)
	{
		double U = 0;
		double delta_t = 0.0;
		// first RTT
		if (qp->intcc.m_lastUpdateSeq == 0)
		{
			...
			for (uint32_t i = 0; i < ih.nhop; i++)
			{
				// header中的int数据存放到hop中, hop[maxHop], maxHop = 5, 在intHeader中
		...
		else
		{
			...
			for (uint32_t i = 0; i < ih.nhop; i++)
			{
				...
				double duration = tau * 1e-9;
				double txRate = (ih.hop[i].GetBytesDelta(qp->intcc.hop[i])) * 8 / duration;
				/**
				 * 1. 取了当前队列长度和记录中队列长度中较小的那个
				 * 2. (qlen/B*T)用(qlen/最大inflghts*速率比例)替代
				 */
				double u = txRate / ih.hop[i].GetLineRate() + (double)std::min(ih.hop[i].GetQlen(), qp->intcc.hop[i].GetQlen()) * qp->m_max_rate.GetBitRate() / ih.hop[i].GetLineRate() / qp->m_win;
	...
	void RdmaHw::UpdateRateIntra(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch, bool fast_react)
	{
