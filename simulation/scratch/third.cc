...
const string PATH_TO_FLOW_FILE = "mix/traffic/";
const string PATH_TO_OUTPUT_FILE = "../data/";
...
string data_rate, link_delay, topology_file, flow_file_name, flow_file, trace_file;
// output file
string trace_output_file;
...
unordered_map<uint32_t, unordered_map<uint32_t, uint16_t> > portNumder;
...
void PrintProgress(Time interval) {
    cout << "Progress to " << Simulator::Now().GetSeconds() << " seconds simulation time" << endl;
...
Ipv4Address node_id_to_ip(uint32_t id) { return Ipv4Address(0x0b000001 + ((id / 256) * 0x00010000) + ((id % 256) * 0x00000100)); }
...
void monitor_buffer(FILE *qlen_output, NodeContainer *n) {
    for (uint32_t i = 0; i < n->GetN(); i++) {
        if (n->Get(i)->GetNodeType() == 1) {  // is switch
            ...
            for (uint32_t j = 1; j < sw->GetNDevices(); j++) {
                uint32_t size = 0;
                for (uint32_t k = 0; k < SwitchMmu::qCnt; k++) size += sw->m_mmu->egress_bytes[j][k];
    ...
    if (Simulator::Now().GetTimeStep() < qlen_mon_end) Simulator::Schedule(NanoSeconds(qlen_mon_interval), &monitor_buffer, qlen_output, n);
}
...
int main(int argc, char *argv[]) {
...
#endif
    {
        ...
        while (!conf.eof()) {
            ...
            if (key.compare("ENABLE_QCN") == 0) {
                ...
                if (use_dynamic_pfc_threshold)
                    cout << "USE_DYNAMIC_PFC_THRESHOLD\t"
                         << "Yes"
                         << "\n";
                else
                    cout << "USE_DYNAMIC_PFC_THRESHOLD\t"
                ...
                conf >> v;
                pause_time = v;
                cout << "PAUSE_TIME\t\t\t" << pause_time << "\n";
            } else if (key.compare("DATA_RATE") == 0) {
                string v;
                conf >> v;
                data_rate = v;
                cout << "DATA_RATE\t\t\t" << data_rate << "\n";
            } else if (key.compare("LINK_DELAY") == 0) {
                string v;
                conf >> v;
                link_delay = v;
                cout << "LINK_DELAY\t\t\t" << link_delay << "\n";
            } else if (key.compare("PACKET_PAYLOAD_SIZE") == 0) {
                uint32_t v;
                conf >> v;
                packet_payload_size = v;
                cout << "PACKET_PAYLOAD_SIZE\t\t" << packet_payload_size << "\n";
            } else if (key.compare("L2_CHUNK_SIZE") == 0) {
                uint32_t v;
                conf >> v;
                l2_chunk_size = v;
                cout << "L2_CHUNK_SIZE\t\t\t" << l2_chunk_size << "\n";
            } else if (key.compare("L2_ACK_INTERVAL") == 0) {
                uint32_t v;
                conf >> v;
                l2_ack_interval = v;
                cout << "L2_ACK_INTERVAL\t\t\t" << l2_ack_interval << "\n";
            } else if (key.compare("L2_BACK_TO_ZERO") == 0) {
                uint32_t v;
                conf >> v;
                l2_back_to_zero = v;
                if (l2_back_to_zero)
                    cout << "L2_BACK_TO_ZERO\t\t\t"
                         << "Yes"
                         << "\n";
                else
                    cout << "L2_BACK_TO_ZERO\t\t\t"
                ...
                conf >> v;
                topology_file = v;
                cout << "TOPOLOGY_FILE\t\t\t" << topology_file << "\n";
            } else if (key.compare("FLOW_FILE_NAME") == 0) {
                string v;
                conf >> v;
                flow_file_name = v;
                flow_file = PATH_TO_FLOW_FILE + v + ".txt";
                cout << "FLOW_FILE_NAME\t\t\t" << flow_file << "\n";
                ...
                cout << "SIMULATOR_STOP_TIME\t\t" << simulator_stop_time << "\n";
            } else if (key.compare("ALPHA_RESUME_INTERVAL") == 0) {
                double v;
                conf >> v;
                alpha_resume_interval = v;
                cout << "ALPHA_RESUME_INTERVAL\t\t" << alpha_resume_interval << "\n";
            } else if (key.compare("RP_TIMER") == 0) {
                double v;
                conf >> v;
                rp_timer = v;
                cout << "RP_TIMER\t\t\t" << rp_timer << "\n";
            } else if (key.compare("EWMA_GAIN") == 0) {
                double v;
                conf >> v;
                ewma_gain = v;
                cout << "EWMA_GAIN\t\t\t" << ewma_gain << "\n";
            } else if (key.compare("FAST_RECOVERY_TIMES") == 0) {
                uint32_t v;
                conf >> v;
                fast_recovery_times = v;
                cout << "FAST_RECOVERY_TIMES\t\t" << fast_recovery_times << "\n";
            } else if (key.compare("RATE_AI") == 0) {
                string v;
                conf >> v;
                rate_ai = v;
                cout << "RATE_AI\t\t\t\t" << rate_ai << "\n";
            } else if (key.compare("RATE_HAI") == 0) {
                string v;
                conf >> v;
                rate_hai = v;
                cout << "RATE_HAI\t\t\t" << rate_hai << "\n";
            } else if (key.compare("ERROR_RATE_PER_LINK") == 0) {
                double v;
                conf >> v;
                error_rate_per_link = v;
                cout << "ERROR_RATE_PER_LINK\t\t" << error_rate_per_link << "\n";
            } else if (key.compare("CC_MODE") == 0) {
                conf >> cc_mode;
                cout << "CC_MODE\t\t" << cc_mode << '\n';
            } else if (key.compare("RATE_DECREASE_INTERVAL") == 0) {
                double v;
                conf >> v;
                rate_decrease_interval = v;
                cout << "RATE_DECREASE_INTERVAL\t\t" << rate_decrease_interval << "\n";
            } else if (key.compare("MIN_RATE") == 0) {
                conf >> min_rate;
                cout << "MIN_RATE\t\t" << min_rate << "\n";
            } else if (key.compare("HAS_WIN") == 0) {
                conf >> has_win;
                cout << "HAS_WIN\t\t" << has_win << "\n";
            } else if (key.compare("GLOBAL_T") == 0) {
                conf >> global_t;
                cout << "GLOBAL_T\t\t" << global_t << '\n';
            } else if (key.compare("MI_THRESH") == 0) {
                conf >> mi_thresh;
                cout << "MI_THRESH\t\t" << mi_thresh << '\n';
                ...
                conf >> u_target;
                cout << "U_TARGET\t\t" << u_target << '\n';
            } else if (key.compare("INT_MULTI") == 0) {
                conf >> int_multi;
                cout << "INT_MULTI\t\t\t\t" << int_multi << '\n';
            } else if (key.compare("RATE_BOUND") == 0) {
                uint32_t v;
                conf >> v;
                rate_bound = v;
                cout << "RATE_BOUND\t\t" << rate_bound << '\n';
            } else if (key.compare("ACK_HIGH_PRIO") == 0) {
                conf >> ack_high_prio;
                cout << "ACK_HIGH_PRIO\t\t" << ack_high_prio << '\n';
            } else if (key.compare("DCTCP_RATE_AI") == 0) {
                conf >> dctcp_rate_ai;
                cout << "DCTCP_RATE_AI\t\t\t\t" << dctcp_rate_ai << "\n";
            } else if (key.compare("LINK_DOWN") == 0) {
                conf >> link_down_time >> link_down_A >> link_down_B;
                cout << "LINK_DOWN\t\t\t\t" << link_down_time << ' ' << link_down_A << ' ' << link_down_B << '\n';
            } else if (key.compare("ENABLE_TRACE") == 0) {
                conf >> enable_trace;
                cout << "ENABLE_TRACE\t\t\t\t" << enable_trace << '\n';
                ...
                conf >> qlen_mon_start;
                cout << "QLEN_MON_START\t\t\t\t" << qlen_mon_start << '\n';
            } else if (key.compare("QLEN_MON_END") == 0) {
                conf >> qlen_mon_end;
                cout << "QLEN_MON_END\t\t\t\t" << qlen_mon_end << '\n';
            } else if (key.compare("MULTI_RATE") == 0) {
                int v;
                conf >> v;
                multi_rate = v;
                cout << "MULTI_RATE\t\t\t\t" << multi_rate << '\n';
            } else if (key.compare("SAMPLE_FEEDBACK") == 0) {
                int v;
                conf >> v;
                sample_feedback = v;
                cout << "SAMPLE_FEEDBACK\t\t\t\t" << sample_feedback << '\n';
            } else if (key.compare("PINT_LOG_BASE") == 0) {
                conf >> pint_log_base;
                cout << "PINT_LOG_BASE\t\t\t\t" << pint_log_base << '\n';
        ...
        cout << "Error: require a config file\n";
        fflush(stdout);
        return 1;
    }
    ...
    cc_mode_name = cc_mode_map[cc_mode];
    ...
    trace_output_file = PATH_TO_OUTPUT_FILE + flow_file_name + "/" + cc_mode_name + ".tr";
