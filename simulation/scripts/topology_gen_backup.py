def auto_mix_topology_gen(config_file_name, num_host_per_rack, num_rack, num_agg, num_core, num_wan_switch, num_wan_host, bandwidth_dchost_to_rack, bandwidth_rack_to_agg, bandwidth_agg_to_core, bandwidth_core_to_wanswitch, bandwidth_wanswitch_to_wanhost, delay_dc, delay_wan, error_rate_dc, error_rate_wan, pod):
    """
    ...
    2) we first number the wan/dc hosts, then number the switches.
    ...
    for i in range(num_wan_switch):
        for j in range(num_wan_host):
            ...
            cnt_link+=1
    # 6.write total node #, switch node #, link # to config file, use r+ to overwrite the first line
